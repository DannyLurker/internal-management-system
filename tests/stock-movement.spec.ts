...
/**
 * ASSUMPTION: a `/api/stocks/:id` GET endpoint exists and returns
 * `{ data: { id, quantity, type, itemId, locationId, expiredAt } }`.
 * This is required to assert quantities actually moved between stock rows
 * (which is the bug this suite is specifically written to catch). If that
 * route doesn't exist yet, swap `getStock()` below for whatever read path
 * you do have (e.g. filtering `/api/stocks?itemId=...`).
 */
...
async function findStockByType(
  ...
  const res = await request.get(
    `/api/stocks?itemId=${itemId}&locationId=${locationId}&type=${type}&page=1&dataPerPage=10`,
  );
  expect(res.status()).toBe(200);
  const body = await res.json();
...
test.describe("CRUD operations for Stock Movements", () => {
  ...
  test.beforeAll(
    ...
    async ({ request }) => {
      ...
      // Seed 100 units of READY stock at the source location via RECEIVE,
      // so later tests (TRANSFER, ADJUSTMENT, MARK_AS_*, quick actions) have
      // a known starting quantity to assert against.
      const receiveResponse = await request.post("/api/stock-movements", {
        data: {
          itemId: testItemId,
      ...
      const seededStock = await findStockByType(
        request,
        testItemId,
        testSourceLocationId,
        "READY",
      );
      console.log(seededStock);
      expect(seededStock).toBeDefined();
  ...
  test(`MARK_AS_DAMAGED called twice accumulates on the same DAMAGED stock row`, async ({
    ...
    await request.post("/api/stock-movements", {
      data: {
        itemId: testItemId,
        stockId: readyStockId,
  ...
  test("quickDiscard moves quantity from READY into the target discard stock", async ({
    ...
    const response = await request.post("/api/stock-movements/quick-discard", {
      data: {
        stockId: readyStockId,
        quantity: 3,
        totalCost: 5000,
  ...
  test("quickDiscard accumulates onto an existing destination stock row instead of creating a duplicate", async ({
    ...
    await request.post("/api/stock-movements/quick-discard", {
      data: {
        stockId: readyStockId,
        quantity: 2,
        totalCost: 3000,
  ...
  test("quickLaundryOut moves quantity from READY into DIRTY", async ({
    ...
    const response = await request.post(
      "/api/stock-movements/quick-laundry-out",
      {
        data: {
          stockId: readyStockId,
          quantity: 5,
